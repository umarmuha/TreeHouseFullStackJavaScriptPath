...
function addNewTeachers(newTeachers) {
    ...
    let allTeachers = teachers.concat(newTeachers);
    return allTeachers;
}

function printTreehouseSummary() {
    // TODO: fix this function so that it prints the correct number of courses and   teachers 
    techersList = addNewTeachers(newTeachers);
    for (let i = 0; i < techersList.length; i++) {
        console.log(`${techersList[i].name} teaches ${techersList[i].topicArea}`);
    }

    console.log(`Treehouse has ${i} JavaScript courses, and ${techersList.length} Javascript teachers`);
}
